...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={handleOpen}
        className="relative flex h-9 w-9 items-center justify-center border border-[rgba(240,236,227,0.15)] text-[#c8c8c0] transition hover:border-[#FFD300] hover:text-[#ede8df]"
      >
        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
        </svg>
        {unread > 0 && (
          <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded bg-[#FFD300] text-[8px] font-bold text-black">
            {unread > 9 ? "9+" : unread}
          </span>
        )}
      </button>
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                ...
                >
                  ...
                  {!n.read && (
                    <div className="mt-1 h-[2px] w-4 bg-[#FFD300]" />
                  )}
                </Link>
              ))
            )}
