      ...
      <article className="border-b border-[#1a1a22]">
        ...
        <div className="relative px-4 pb-4">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <Link href="/clan" className="mt-2 text-[12px] font-medium text-[#FFFF01]">
              Кланд нэгдэх →
            </Link>
          </div>
        </div>
        ...
        <Link href={`/profile/${post.author._id}`} className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {post.category === "ялалт" && (
                <span className="rounded-[4px] bg-[rgba(255,255,1,0.1)] px-1.5 py-0.5 text-[9px] font-bold tracking-wide text-[#FFFF01]">ЯЛАЛТ</span>
              )}
            </div>
          </div>
        </Link>
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-1 rounded-[4px] px-1.5 py-1 transition-all ${
                  reacted
                    ? "bg-[rgba(255,255,1,0.08)] shadow-[0_0_6px_rgba(255,255,1,0.08)]"
              ...
              >
                ...
                {count > 0 && (
                  <span className={`text-[10px] font-semibold tabular-nums ${reacted ? "text-[#FFFF01]" : "text-[#4a4a55]"}`}>
                    {count}
                  </span>
                )}
              </button>
