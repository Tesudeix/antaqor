...
// Different caps for users vs guests. Guests are cheaper to rate-limit
// because we want xAI cost predictable while still letting visitors taste
// the experience.
...
const GUEST_LIFETIME = 30; // total messages per guestKey before signup nudge
...
export async function POST(req: NextRequest) {
  ...
  if (!subject) {
    return NextResponse.json(
      { error: "Танигдсангүй. Хуудсаа дахин ачаалаад үзнэ үү." },
      { status: 401 }
    );
  }

  // Rate limits — different caps per subject kind
  const subjKey = subjectRateKey(subject);
  if (subject.kind === "user") {
    const burst = rateLimit(`companion:burst:${subjKey}`, USER_PER_MIN);
    if (!burst.ok) return tooMany("Дэндүү хурдан бичлээ.", burst.resetInMs);
    const day = rateLimit(`companion:day:${subjKey}`, USER_PER_DAY);
    if (!day.ok) return tooMany(`Өнөөдрийн чат хязгаар (${day.limit}) хүрлээ.`, day.resetInMs);
  } else {
    const burst = rateLimit(`companion:burst:${subjKey}`, GUEST_PER_MIN);
    if (!burst.ok) return tooMany("Хэдхэн секунд хүлээгээрэй.", burst.resetInMs);
    const hr = rateLimit(`companion:hour:${subjKey}`, GUEST_PER_HOUR);
    if (!hr.ok) return tooMany(`Зочин 1 цагт ${hr.limit} мессеж. Бүртгүүлбэл хязгааргүй.`, hr.resetInMs);
  }
  ...
  const [memDoc, recentDocs, userDoc] = await Promise.all([
    CompanionMemory.findOne(filter),
    CompanionMessage.find(filter).sort({ createdAt: -1 }).limit(RECENT_TURNS).lean(),
    subject.kind === "user" ? User.findById(subject.userId).select("name").lean() : Promise.resolve(null),
  ]);

  const memory = memDoc || (await CompanionMemory.create({ ...insert }));

  // Lifetime cap for guests — protect xAI credit cost without breaking the
  // first-touch experience entirely.
  if (subject.kind === "guest" && (memory.totalMessages || 0) >= GUEST_LIFETIME) {
    return NextResponse.json(
      {
        error: `Зочин ${GUEST_LIFETIME} мессеж хүртэл боломжтой. Бүртгүүлбэл хязгааргүй ярина.`,
        signupRequired: true,
      },
      { status: 402 }
    );
  ...
  // Persist user msg first so a Grok timeout doesn't lose it
...
function tooMany(message: string, resetInMs: number) {
  ...
  return NextResponse.json(
    { error: `${message} ${sec}с дараа дахин үзнэ үү.` },
