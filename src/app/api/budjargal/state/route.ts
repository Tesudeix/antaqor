...
import { depthBand } from "@/lib/budjargal";
...
export async function GET(req: NextRequest) {
  ...
  if (!subject) return NextResponse.json({ error: "Танигдсангүй", isGuest: false }, { status: 401 });
  await dbConnect();

  const filter = subjectFilter(subject);
  const [memDoc, messages] = await Promise.all([
    BudjargalMemory.findOne(filter).lean(),
    BudjargalMessage.find(filter).sort({ createdAt: -1 }).limit(PAGE).lean(),
  ]);
  ...
  const band = depthBand(affection);
