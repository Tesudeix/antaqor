...
export default function MembersPage() {
        ...
        <div className="px-5 pb-4 pt-2">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1 text-[28px] font-bold leading-none tracking-tight text-[#1A1A1A] sm:text-[36px]">Гишүүд</h1>
            </div>
            <div className="flex items-center gap-1.5 pb-1">
              <div className="h-1.5 w-1.5 rounded-[4px] bg-[#EF2C58]" />
              <span className="text-[22px] font-bold tabular-nums text-[#1A1A1A]">—</span>
            </div>
          </div>
        </div>
      ...
      <div className="-mx-5">
        ...
        <div className="px-5 pb-4 pt-2">
          <div className="flex items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-1.5 pb-1">
              <div className="h-1.5 w-1.5 rounded-[4px] bg-[#EF2C58]" />
              <span className="text-[22px] font-bold tabular-nums text-[#1A1A1A] sm:text-[28px]">{totalMembers}</span>
            </div>
          </div>
        </div>

        {/* CTA */}
        <div className="mx-5 rounded-[4px] border border-[rgba(0,0,0,0.08)] bg-[#FFFFFF] p-6 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-[13px] leading-relaxed text-[#888888]">
          ...
          {isLoggedIn ? (
            ...
            <div className="mt-5 flex items-center justify-center gap-3">
              ...
              <Link href="/auth/signin" className="rounded-[4px] border border-[rgba(0,0,0,0.08)] px-5 py-2.5 text-[13px] font-medium text-[#888888] transition hover:text-[#1A1A1A]">
                Нэвтрэх
              </Link>
            </div>
          )}
              ...
              {members.slice(0, 5).map((member) => {
                ...
                return (
                  <div key={member._id} className="flex items-center gap-3 py-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[4px] bg-[rgba(0,0,0,0.08)] text-[11px] font-bold text-[#888888]">
                      {initials}
                    </div>
                    <div>
  ...
  return (
    <div className="-mx-5">
      ...
      {/* Search + Chat link */}
      <div className="sticky top-[57px] z-30 border-b border-t border-[rgba(0,0,0,0.08)] bg-[rgba(6,6,8,0.92)] px-5 py-3 backdrop-blur-lg">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-[4px] bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)] py-2 pl-9 pr-3 text-[13px] text-[#1A1A1A] placeholder-[#AAAAAA] outline-none transition focus:border-[#EF2C58]/50"
            />
            {search && (
              <button onClick={() => setSearch("")} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#AAAAAA] transition hover:text-[#1A1A1A]">
          ...
          <Link
            href="/chat"
            className="flex items-center gap-1.5 rounded-[4px] border border-[rgba(0,0,0,0.08)] px-3 py-2 text-[12px] font-medium text-[#888888] transition hover:border-[#2a2a35] hover:text-[#1A1A1A]"
          >
      ...
      <div className="px-5 pt-2">
        {filtered.length === 0 && search ? (
          <div className="py-16 text-center">
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-[4px] bg-[#FFFFFF] border border-[rgba(0,0,0,0.08)]">
              <svg className="h-5 w-5 text-[#AAAAAA]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </div>
            <p className="text-[14px] font-medium text-[#888888]">&ldquo;{search}&rdquo; илэрц олдсонгүй</p>
...
function MemberRow({ member, index }: { member: Member; index: number }) {
  ...
  return (
    <Link
      ...
      className="group flex animate-fade-up items-center gap-3 py-3 transition active:bg-[#FFFFFF] sm:gap-3.5"
    ...
    >
        ...
        {member.avatar ? (
          ...
          <div
            className={`flex h-11 w-11 items-center justify-center rounded-[4px] text-[12px] font-bold sm:h-12 sm:w-12 ${
              admin ? "bg-[#EF2C58] text-white" : "bg-[rgba(0,0,0,0.08)] text-[#888888]"
            }`}
          >
            {initials}
          </div>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className={`truncate text-[14px] font-semibold transition group-hover:text-[#EF2C58] ${admin ? "text-[#EF2C58]" : "text-[#1A1A1A]"}`}>
            {member.name}
          </p>
          {admin && (
        ...
        <div className="mt-0.5 flex items-center gap-2">
          ...
          {member.bio && (
            <>
              ...
              <p className="truncate text-[11px] text-[#888888]">{member.bio}</p>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        ...
        {!isMe && (
          <button
            onClick={startChat}
            className="flex h-8 w-8 items-center justify-center rounded-[4px] border border-[rgba(0,0,0,0.08)] text-[#AAAAAA] transition hover:border-[#EF2C58]/30 hover:text-[#EF2C58]"
            title="Мессеж"
          >
            <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
