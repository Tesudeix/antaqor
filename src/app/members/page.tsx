...
export default function MembersPage() {
      ...
      <div className="-mx-5">
        <div className="px-5 pb-4 pt-2">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-[11px] font-medium uppercase tracking-[2px] text-[#3a3a48]">Нийгэмлэг</p>
    ...
    return (
      <div className="-mx-5">
        ...
        <div className="mx-5 rounded-[4px] border border-[#1a1a22] bg-[#0c0c10] p-6 text-center">
          ...
          <h2 className="text-[18px] font-bold text-[#e8e6e1]">
              ...
              {members.slice(0, 5).map((member) => {
                ...
                return (
                  <div key={member._id} className="flex items-center gap-3 py-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[4px] bg-[#1a1a22] text-[11px] font-bold text-[#6b6b78]">
                      {initials}
                    </div>
                    <div>
                      <div className="h-3 w-20 rounded bg-[#1a1a22]" />
                      <div className="mt-1.5 h-2.5 w-14 rounded bg-[#1a1a22]" />
                    </div>
                  </div>
                );
              })}
  ...
  return (
    <div className="-mx-5">
      ...
      {/* Search + Chat link */}
      <div className="sticky top-[57px] z-30 border-b border-t border-[#1a1a22] bg-[rgba(6,6,8,0.92)] px-5 py-3 backdrop-blur-lg">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <svg className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#3a3a48]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Link
            href="/chat"
            className="flex items-center gap-1.5 rounded-[4px] border border-[#1a1a22] px-3 py-2 text-[12px] font-medium text-[#6b6b78] transition hover:border-[#2a2a35] hover:text-[#e8e6e1]"
          >
        ...
        {search && (
          <p className="mt-2 text-[11px] text-[#3a3a48]">{filtered.length} илэрц</p>
        )}
      </div>
      ...
      <div className="px-5 pt-2">
        {filtered.length === 0 && search ? (
          <div className="py-16 text-center">
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-[4px] bg-[#0c0c10] border border-[#1a1a22]">
              <svg className="h-5 w-5 text-[#3a3a48]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
              </svg>
            </div>
            <p className="text-[14px] font-medium text-[#6b6b78]">&ldquo;{search}&rdquo; илэрц олдсонгүй</p>
            <p className="mt-1 text-[12px] text-[#3a3a48]">Өөр нэрээр хайж үзнэ үү</p>
          </div>
        ) : (
...
function MemberRow({ member, index }: { member: Member; index: number }) {
  ...
  return (
    <Link
      ...
      className="group flex animate-fade-up items-center gap-3 py-3 transition active:bg-[#0c0c10] sm:gap-3.5"
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2">
          ...
          {member.bio && (
            <>
              <span className="text-[10px] text-[#1a1a22]">·</span>
              <p className="truncate text-[11px] text-[#6b6b78]">{member.bio}</p>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        {member.clanJoinedAt && (
          <span className="hidden text-[10px] text-[#3a3a48] sm:block">
    ...
    <div className="divide-y divide-[#1a1a22]">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 py-3">
          <div className="h-11 w-11 shrink-0 animate-pulse rounded-[4px] bg-[#1a1a22]" />
          <div className="flex-1 space-y-2">
            <div className="h-3 w-24 animate-pulse rounded-[4px] bg-[#1a1a22]" />
            <div className="h-2.5 w-36 animate-pulse rounded-[4px] bg-[#1a1a22]" />
          </div>
        </div>
      ))}
    </div>
